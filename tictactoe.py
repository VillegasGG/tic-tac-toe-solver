...
class TicTacToeEnv(Env):
    ...
    def exists_winner(self):
        """
        Returns integer of the winner of the game. 0 if no winner yet.
        ...
        if(self._row_winner(self._obs) != 0):
            winner = self._row_winner(self._obs)
        elif(self._col_winner(self._obs) != 0):
            winner = self._col_winner(self._obs)
        elif(self._main_diag_winner(self._obs) != 0):
            winner = self._main_diag_winner(self._obs)
        elif(self._reverse_main_diag_winner(self._obs) != 0):
    ...
    def _is_game_over(self):
        ...
        if len(self._history) == self.action_space.n:
            self._winner = self.exists_winner()
            return True
