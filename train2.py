...
def play(rounds=20000, show=False):
    ...
    # Load policy for second agent
    policy = 'policy2.pkl'
    with open(policy, 'rb') as f:
        policy_data = pickle.load(f)

    # Create opponent agent with loaded policy
    if policy_data is None:
        print(f'Policy file {policy} not found. Using default policy.')
        opponent_agent = AgentPlayer('OpponentBot', epsilon=0.1)
    else:
        print(f'Loading policy from {policy}')
        opponent_agent = AgentPlayer('OpponentBot', epsilon=0.1, policy_data=policy_data)
